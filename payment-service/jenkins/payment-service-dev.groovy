      ...
      steps {
        script {
          ...
          if (!(branch == 'develop' || branch.startsWith('feat/'))) {
            error "payment-service-dev solo se ejecuta en ramas develop o feat/** (rama actual: '${branch}')."
          }
...
'''
  ...
  post {
    success {
      echo "✅ payment-service-dev completado. Resultados almacenados en reports/test-reports-payment-service.tar.gz (si aplica)."
      script {
        try {
          sh("""curl -X POST "https://api.github.com/repos/OscarMURA/ecommerce-microservice-backend-app/statuses/${env.GIT_COMMIT}" \\
            ...
            -d '{"state":"success","description":"Jenkins: Build passed","context":"ci/jenkins/payment-service"}'""")
        } catch (Exception e) {
    ...
    failure {
      echo "❌ payment-service-dev falló. Revisa los logs para detalles."
      script {
        try {
          sh("""curl -X POST "https://api.github.com/repos/OscarMURA/ecommerce-microservice-backend-app/statuses/${env.GIT_COMMIT}" \\
            -H "Authorization: token \${GITHUB_TOKEN}" \\
            -H "Content-Type: application/json" \\
            -d '{"state":"failure","description":"Jenkins: Build failed","context":"ci/jenkins/payment-service"}'""")
        } catch (Exception e) {
          echo "⚠️ No se pudo actualizar estado en GitHub: ${e.message}"
        }
      }
