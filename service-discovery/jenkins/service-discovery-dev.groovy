      ...
      steps {
        script {
          ...
          if (!(branch == 'develop' || branch.startsWith('feat/'))) {
            error "service-discovery-dev solo se ejecuta en ramas develop o feat/** (rama actual: '${branch}')."
          }
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  HEALTH_RESPONSE=$(kubectl exec -n "$NAMESPACE" deployment/"$SERVICE_NAME" -- curl -s http://localhost:"$SERVICE_PORT"/actuator/health 2>/dev/null || echo "ERROR")
      ...
      script {
        try {
          sh("""curl -X POST "https://api.github.com/repos/OscarMURA/ecommerce-microservice-backend-app/statuses/${env.GIT_COMMIT}" \\
            ...
            -d '{"state":"success","description":"Jenkins: Build passed","context":"ci/jenkins/service-discovery"}'""")
        } catch (Exception e) {
      ...
      script {
        try {
          sh("""curl -X POST "https://api.github.com/repos/OscarMURA/ecommerce-microservice-backend-app/statuses/${env.GIT_COMMIT}" \\
            -H "Authorization: token \${GITHUB_TOKEN}" \\
            -H "Content-Type: application/json" \\
            -d '{"state":"failure","description":"Jenkins: Build failed","context":"ci/jenkins/service-discovery"}'""")
        } catch (Exception e) {
          echo "⚠️ No se pudo actualizar estado en GitHub: ${e.message}"
        }
      }
