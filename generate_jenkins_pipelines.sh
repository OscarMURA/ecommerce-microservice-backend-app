...
# Script para generar pipelines de Jenkins para cada microservicio
# Basado en el pipeline de user-service
...
# Función para generar el pipeline de un servicio
generate_pipeline() {
  local service_name="$1"
  local pipeline_file="$service_name/jenkins/${service_name}-dev.groovy"
  
  echo "Generando pipeline para $service_name..."
      ...
      steps {
        script {
          ...
          if (!(branch == 'develop' || branch.startsWith('feat/'))) {
            error "SERVICE_PLACEHOLDER-dev solo se ejecuta en ramas develop o feat/** (rama actual: '${branch}')."
          }
...
'''
  ...
  post {
    success {
      echo "✅ SERVICE_PLACEHOLDER-dev completado. Resultados almacenados en reports/test-reports-SERVICE_PLACEHOLDER.tar.gz (si aplica)."
    }
    failure {
      echo "❌ SERVICE_PLACEHOLDER-dev falló. Revisa los logs para detalles."
    }
    always {
      cleanWs()
    }
...
EOF
  ...
  # Reemplazar placeholders con el nombre del servicio
  sed -i "s/SERVICE_PLACEHOLDER/$service_name/g" "$pipeline_file"
  
  echo "✅ Pipeline generado: $pipeline_file"
}

# Generar pipelines para todos los servicios
for service in "${SERVICES[@]}"; do
  generate_pipeline "$service"
done

echo "🎉 Todos los pipelines han sido generados exitosamente!"
echo ""
echo "Pipelines creados:"
for service in "${SERVICES[@]}"; do
  echo "  - $service/jenkins/${service}-dev.groovy"
done
